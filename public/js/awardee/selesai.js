document.addEventListener('DOMContentLoaded', async function (){
    ...
    const dataSelesai = await fetch('/dataPenugasanSelesai', {
        method: 'GET'
    })
    const response = await dataSelesai.json()
    if (response.success) {
        ...
        for (let index = 0; index < response.results.length; index++) {
            ...
            div5.appendChild(a1)
            a1.textContent = `${response.results[index].judul_materi}`
            a1.setAttribute('id_instruksi', `${response.results[index].id_instruksi}`)
            a1.setAttribute('id_pengumpulan', `${response.results[index].id_pengumpulan}`)
            a1.setAttribute('id_awardee', `${response.results[index].id_awardee}`)

            const dateTimeString = `${response.results[index].created_at}`;
    ...
    for (let index = 0; index < btnDetail.length; index++) {
        ...
        btnDetail[index].addEventListener('click', async function () {
            const id_pengumpulan = btnDetail[index].getAttribute('id_pengumpulan')
            const id_instruksi = btnDetail[index].getAttribute('id_instruksi')
            const id_awardee = btnDetail[index].getAttribute('id_awardee')
            window.location.href = `/detailSelesaiView/${id_instruksi}/${id_pengumpulan}/${id_awardee}`
        })
    }
